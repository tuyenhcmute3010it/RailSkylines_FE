...
export default function OrderTable() {
  ...
  useEffect(() => {
    ...
    function onNewOrder(data: GuestCreateOrdersResType["data"]) {
      const { guest } = data[0];
      toast({
        description: `Guest : ${guest?.name} in table [${guest?.tableNumber}] just order ${data.length} dish`,
      });
      refetch();
    }
    function onPayment(data: PayGuestOrdersResType["data"]) {
      const { guest } = data[0];
      toast({
        description: `${guest?.name} in table [${guest?.tableNumber}] đã thanh toán thành công ${data.length} dish`,
      });
      refetch();
    }
    ...
    return () => {
      ...
      socket.off("new-payment", onPayment);
    };
