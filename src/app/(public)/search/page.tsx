...
export default function Search() {
  ...
  // Compute updatedSeatData at top level
  const updatedSeatData = useMemo(() => {
    if (!availableSeatsData || !selectedCoach) return [];
    const filteredSeats = availableSeatsData.filter(
      (seat: Seat) => seat?.carriage?.carriageId === selectedCoach.id
    );
    return filteredSeats.map((seat: Seat) => ({
      ...seat,
  ...
  useEffect(() => {
    ...
    // Only update selectedCoach if seatData has changed
    setSelectedCoach((prev) => {
      if (!prev) return prev;
      const currentSeatIds = prev.seatData.map((seat) => seat.seatId).sort();
      const newSeatIds = updatedSeatData.map((seat) => seat.seatId).sort();
      if (
        currentSeatIds.length === newSeatIds.length &&
        currentSeatIds.every((id, index) => id === newSeatIds[index])
      ) {
  ...
  }, [availableSeatsData, availableSeatsError, updatedSeatData]); // Removed selectedCoach from dependencies
  ...
  const renderSeat = useCallback(
    (seat: number, type: "seat" | "4-bed" | "6-bed") => {
      if (!selectedCoach) return null;
      const seatData = selectedCoach.seatData.find((s) => s.seatId === seat);
      const price = seatData ? seatData.price : selectedCoach.basePrice;
      const isUnavailable = seatData?.seatStatus !== "available";
      const isPending = seatData?.seatStatus === "pending";
      const isSelected = getCurrentSelectedSeats().includes(seat);
      ...
      return (
        <div key={seat} className="relative group">
          <Card
            ...
            } flex items-center justify-center ${bgColor} ${textColor} ${cursor}`}
            style={{
              backgroundImage: `url('${
                type === "seat" ? "/seat.png" : "/bed.png"
              }')`,
            }}
            onClick={() => isInteractable && toggleSeatSelection(seat)}
          >
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block bg-gray-800 text-white text-xs rounded py-1 px-2 z-10">
            {formatPrice(price)}{" "}
            {isUnavailable && (isPending ? "(Đang chờ)" : "(Không khả dụng)")}
          </div>
        </div>
      );
    },
    [selectedCoach, getCurrentSelectedSeats, toggleSeatSelection, formatPrice]
  );

  const renderSixBedSection = useCallback(() => {
    if (!selectedCoach || selectedCoach.type !== "6-bed") return null;
    console.log("renderSixBedSection called, selectedCoach:", selectedCoach);
    return selectedCoach.seatData
      .map((seat) => renderSeat(seat.seatId, "6-bed"))
      .filter(Boolean);
  }, [renderSeat, selectedCoach]);
          ...
          <div className="border-4 border-[#385d8a] p-4 mt-2 rounded-lg">
            ...
            {isSeatsLoading ? (
              ...
              <div className="grid grid-cols-[repeat(7,minmax(0,1fr))] gap-4">
                {selectedCoach.type === "6-bed" && renderSixBedSection()}
              </div>
            )}
          </div>
        )}
