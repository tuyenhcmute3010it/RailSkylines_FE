    ...
    .map((trip) => {
      const carriages = trip.train.carriages
        ...
        .map((carriage) => {
          ...
          // Generate full seat list for sixBeds (seat IDs 463–504)
          ...
          if (carriage.carriageType === "sixBeds") {
            ...
            fullSeats = Array.from({ length: maxSeats }, (_, i) => {
              const seatId = 463 + i; // Start from 463
              ...
              return (
                existingSeat || {
                  seatId,
                  seatNumber: seatId,
                  seatType: "LEVEL_3", // Default type
                  seatStatus:
                    seatId === 463 || seatId === 464
                      ? "unavailable"
                      : "available",
                  price: 490000, // Default price for LEVEL_3
                  tickets: [],
                  carriage: {
...
export default function Search() {
  ...
  const derivedSeatData = useMemo(() => {
    ...
    // Generate full seat list (463–504 for sixBeds)
    const fullSeats = Array.from({ length: 42 }, (_, i) => {
      const seatId = 463 + i;
      ...
      return (
        apiSeat || {
          seatId,
          seatNumber: seatId,
          seatType: "LEVEL_3", // Default for missing seats
          seatStatus:
            seatId === 463 || seatId === 464 ? "unavailable" : "available",
          price: 490000, // Default price
          tickets: [],
          carriage: {
            carriageId: selectedCoach.id,
            carriageType:
              selectedCoach.type === "6-bed" ? "sixBeds" : selectedCoach.type,
            price: selectedCoach.basePrice,
            discount: selectedCoach.discount,
            train: {
              trainId: selectedTrain?.trainTripId || 0,
              trainName: selectedTrain?.name || "",
              trainStatus: "active",
            },
          },
    ...
    debounce(async (seat: number) => {
      ...
      if (!isDeselecting) {
        ...
        const newCartItem: CartItem = {
          trainId: selectedTrain.id,
          trainName: selectedTrain.name,
          trainTripId: selectedTrain.trainTripId, // Fixed typo
          coachName: selectedCoach.name,
          coachId: selectedCoach.id,
          seatNumber: seat,
          departure: selectedTrain.departure,
